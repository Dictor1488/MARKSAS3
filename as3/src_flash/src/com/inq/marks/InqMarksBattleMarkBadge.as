package com.inq.marks
{
 ...
 public class InqMarksBattleMarkBadge extends Sprite
 {
  ...
  private function swap():void{drop();if(s==1)r=new InqMarksCompactRenderer();else if(s==2)r=new InqMarksPolaroidRenderer();else if(s==3)r=new InqMarksNeerBadgeRenderer();else if(s==4)r=new InqMarksMinimalBadgeRenderer();else r=new InqMarksClassicRenderer();r.visible=true;r.setExpanded(ex);r.setPositionOffset(off);r.addEventListener(InqMarksPanelEvent.BATTLE_BADGE_OFFSET_CHANGED,onOff);addChild(r);apply();r.updatePosition();}
  private function apply():void{if(r)r.setData(m,p65,p85,p95,p100,cur,base,stars,pm,pa);}
  private function drop():void{if(!r)return;r.removeEventListener(InqMarksPanelEvent.BATTLE_BADGE_OFFSET_CHANGED,onOff);if(r.parent==this)removeChild(r);r.dispose();r=null;}
